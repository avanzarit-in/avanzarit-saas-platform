 ...
 * {@link StructuredTableName} representation. Next to being able to parse table names it is also capable of creating
 * unstructured String table names based on structured content.
 */
public class StructuredTableNameParser {
     ...
     * </ul>
     */
    public StructuredTableName parse(String tableName) {
        StructuredTableName result = null;
        if (tableName != null) {
            String[] split = tableName.split(SPLIT_CHARACTER);
            if (split.length == 4) {
                result = new StructuredTableName();
                result.setProject(split[0]);
                result.setEnvironment(split[1]);
                result.setLayer(split[2]);
                result.setTable(split[3]);
            }
            //serializers
            if (split.length == 5) {
                result = new StructuredTableName();
